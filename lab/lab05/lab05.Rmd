...
### `sample` for data frames

Base R's `sample` function can be used to randomly sample row indices from a data frame. We can then use these indices to subset the data frame. This approach draws at random **without replacement** from the rows of a data frame by default.

Run the cell below to see an example of sampling 5 rows without replacement.

```{r sample_example}
# Just run this cell
set.seed(123)  # for reproducible results
sample_indices <- sample(nrow(salary_data), 5)
salary_data[sample_indices, ]
```
